...
<?
//정렬순서 정하기
if($_GET['rent_type_year']) {//사글세일 경우
	if($_GET['order'] == "desc")//높은 가격순
		$order_query = "order by r_pay_year desc";
	else//낮은가격순
		$order_query = "order by r_pay_year asc";
} elseif($_GET['rent_type_month']) {
	if($_GET['order'] == "desc")
		$order_query = "order by r_pay_month desc";
	else
		$order_query = "order by r_pay_month asc";
} elseif($_GET['rent_type_jun']) {
	if($_GET['order'] == "desc")
		$order_query = "order by r_pay_jun desc";
	else
		$order_query = "order by r_pay_jun asc";
} else {
	if($_GET['order'] == "desc")//높은 가격순
		$order_query = "order by r_pay_year desc";
	else//낮은가격순
...
if($_GET['area_s'] || $_GET['area_n'] || $_GET['rent_type_year'] || $_GET['rent_type_month'] || $_GET['rent_type_jun'] || $_GET['room_type_one'] || $_GET['room_type_two'] || $_GET['room_type_mini'] || $_GET['room_type_etc'] || $_GET['midnight'] || $_GET['tax_b'] || $_GET['tax_e'] || $_GET['tax_w'] || $_GET['tax_g'] || $_GET['tax_i']) {//검색옵션을 하나라도 체크했을 경우
	if($pay_a && $pay_b) { //가격범위를 입력했을 경우
			$room_string = "select * from room ".$query_area.$query_rent_type.$query_room_type.$query_tax." and (r_pay_jun >= ".$pay_a." and r_pay_jun <= ".$pay_b.") ".$order_query;
	} else {
			$room_string = "select * from room ".$query_area.$query_rent_type.$query_room_type.$query_tax.$order_query;
	}
} elseif($pay_a && $pay_b) {//검색옵션 체크 안하고 가격만 체크했을 경우
		if($_GET['rent_type_year']) //사글세
		$room_string = "select * from room where (r_pay_year >= ".$pay_a." and r_pay_year <= ".$pay_b.") ".$order_query;
} else {//검색옵션 미기재시
	$room_string = "select * from room ".$order_query;
}

if($_GET['r_name']) {//방이름 검색
...
?>
<a style="color:#333; text-decoration:none;" href="javascript:popupCenter_b('./room/view-dk.php?r_num=<?echo $data2['r_num']; ?>',900,800)">
